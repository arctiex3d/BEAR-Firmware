...
/*	if (mcu & 1) SERIAL_ECHOLNRPGM(MSG_POWERUP);
	...
	if (extruder_altfan_detect())
		SERIAL_ECHOLNRPGM(PSTR("SUNON/ALTFAN"));
	else
...
if(eSoundMode!=e_SOUND_MODE_SILENT)
    ...
    */
    case 4: 
	  ...
	  if(codenum != 0) LCD_MESSAGERPGM(_n("Waiting..."));////MSG_DWELL
      st_synchronize();
      codenum += _millis();  // keep track of when we started waiting
      previous_millis_cmd = _millis();
  ...
  case 76: 
	{
        ...
        {
            SERIAL_ECHOLNPGM("No Probe thermistor");
            break;
        }
        ...
        SERIAL_ECHOLNPGM("ATCIP Probe calibration start");
        ...
        {
            float temp = (40 + i * 5);
            printf_P(_N("\nStep: %d/6 (skipped)\nATCIP temperature: %d Z shift (mm):0\n"), i + 2, (40 + i*5));
        ...
        for (i++; i < 5; i++)
        {
            ...
            printf_P(_N("\nATCIP temperature: %.1f Z shift (mm): %.3f"), current_temperature_pinda, current_position[Z_AXIS] - zero_z);
		...
		puts_P(_N("ATCIP probe calibration start"));
		custom_message_type = CustomMsg::TempCal;
		custom_message_state = 1;
		lcd_setstatuspgm(_T(MSG_TEMP_CALIBRATION));
		current_position[X_AXIS] = PINDA_PREHEAT_X;
	...
	*/
	case 115: // M115
      if (code_seen('V')) {
          ...
          SERIAL_ECHOPGM("FIRMWARE_NAME:3DXFAB-Firmware ");
          SERIAL_ECHORPGM(FW_VERSION_STR_P());
          SERIAL_ECHOPGM(" based on Marlin FIRMWARE_URL:https://github.com/3DXFAB/3DXFAB-Firmware PROTOCOL_VERSION:");
